...
0.097054, 0.0970329, 0.0970118, 0.0969907]
    ...
    xi = np.linspace(0.32,0.95,15)
    yi = np.linspace(-0.071,inv(5),20)
    
    zi = griddata((Delta, U), sM, (xi[None,:], yi[:,None]), method='linear')

    ax2.contour(xi,yi,zi,100,linewidths=0,colors='k')
    cntr2 = ax2.contourf(xi,yi,zi,100,cmap='Oranges')
    ...
    pointsize=3
    yerrs = [0.0]*10
    yerrs += [0.05/(7.6*7.6), 0.05/(7.44*7.44), 0.1/(7.4*7.4), 0.1/(7.45*7.45)] # \Delta \frac{1}{x} = \frac{1}{x^2} \Delta x
    yerrs += [0]*10
